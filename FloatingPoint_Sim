...
def f_point(reg):
    ...
    f_reg = reg[8:]
    exp = f_reg[0:3]
    exp_dec = bi_to_dec(exp)
    mantissa = f_reg[3:] 
    for i in range(5):
        sumf = sumf + mantissa[i]*(2**(-1-i))
    sumf = sumf + 1
    ans_final = sumf * (2**exp_dec)
    return ans_final
...
def convert(decimal):
    ...
    final = ''
    dec1 = decimal % 1
    integer = int(decimal/1)
    int_f = dec_to_bi(int(integer))
    l = len(int_f)
    while dec1 != 0:
        dec2 = dec1 * 2
        dec1 = dec2 % 1
        ...
        st = st + str(k)
    int_f = int_f + '.' + st    
    ans = float(int_f) / (10 ** (l-1))
    ans = str(ans)  
    ans = ans[2:]
    final = final + dec_to_bi(int(l-1))
    if(len(final)>3 or len(ans)>5):
        return -1
    final = '0'*(3-len(final)) + final + ans + '0'*(5-len(ans)) 
    return final
...
A_sim = {'10000' : 'add', '10001' : 'sub', '10110': 'mul', '11010': 'xor', '11011': 'or', '11100': 'and', '00000': 'addf', '00001': 'subf'}

B_sim = {'10010' : 'mov', '11000': 'rs', '11001': 'ls', '00010': 'movf'}
...
while i < len_sim:
    ...
    if(op_sim in A_sim):
        ...
        if(A_sim[op_sim] == 'add'):
            ...
            if (value1+value2>255):
                reg_value['111'] = '0000000000001000'
        ...
        elif(A_sim[op_sim] == 'addf'):       #incomplete
            value1 = f_point(int(reg_value[a1]))
            value2 = f_point(int(reg_value[a2]))
            sum = value1 + value2
            ansf = convert(sum)
            if (ansf == -1):
                reg_value['111'] = '0000000000001000'
                reg_value[a3] = '0000000011111111'
            else:
                reg_value[a3] = '0'*8 + ansf

        elif(A_sim[op_sim] == 'subf'):          #incomplete
            value1 = f_point(int(reg_value[a1]))
            value2 = f_point(int(reg_value[a2]))
            diff = value1-value2
            if(diff < 1):
    ...
    #Type B
        ...
        elif(B_sim[op_sim] == 'movf'):        #doubtful
            reg_value[b1] = '00000000' + input_sim[i][8:]
            reg_value['111'] = '0000000000000000'
